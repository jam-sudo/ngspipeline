...
- Alternative prebuilt index: `kallisto index` (kallisto bundled in the same kb-python container) on Ensembl 116 cDNA + ncRNA FASTA (`Homo_sapiens.GRCh38.cdna.all.fa.gz` `22380 179589`, `ncrna.fa.gz` `24013 40053`; 669,547 transcripts; `t2g.txt` derived from the FASTA headers `transcript<TAB>gene<TAB>gene_symbol`). Result (2026-09-15, 8 threads, alone in the 20 GB VM): **297 s wall, peak container memory ≈ 15.7 GiB, `index.idx` 1.28 GB** (206 M k-mers, k = 31). A first attempt that overlapped with a pipeline test run was OOM-killed at 13.4 GB RSS, so the build must run alone. Index at `~/ngs_ref/kallisto_cdna_GRCh38_ens116/{index.idx,t2g.txt}`; command in `bin/build_reference_index.sh`.
